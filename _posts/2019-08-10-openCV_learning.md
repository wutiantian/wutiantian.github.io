...
当需要向项目中添加 .dll动态库时，知己将需要添加的 .dll文件拖拽到项目生成的 .exe 所在的文件夹即可。【优先采用统一目录下面的 dll 文件】
...
画面识别，实际上是寻找人类的视觉关联方式 f ，并再次应用。——日本会津大学博士：于建国
...
在 Visual studio 中如果使用 argc 和argv，且没有再 【项目属性】-【配置属性】-【调试】-【命令参数】中指定参数的值，**就会报错**。这是研究OpenCV官方提供的**示例程序时经常碰到的错误**。例如：
